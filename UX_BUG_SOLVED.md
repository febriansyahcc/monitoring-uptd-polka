...
3. **Verifikasi** harus menyebut cara uji yang nyata: langkah manual di browser (role, halaman, input), perintah yang dijalankan (`php artisan test`, `npm run build`), atau keduanya. Jangan menulis "sudah dicek" tanpa langkah.
...
5. Bukti visual (screenshot) opsional; simpan di `docs/bug-evidence/BUG-xx-*.png` dan tautkan.
6. Setelah mengisi detail, **perbarui juga tabel Ringkasan dan Progres** di bawah.
7. Centang juga checklist di `UX_BUG_REPORT.md` bagian *Rencana Pengerjaan* agar keduanya sinkron.
...
| Fase 1 — Bug data & keamanan | 5 | 0 | 🔄 Dikerjakan — 5/5 diimplementasi, di-commit & lolos tes otomatis; menunggu uji browser manual |
...
>
> **Belum dilakukan:** uji manual di browser. Uji ini butuh login dengan password, jadi dikerjakan oleh developer. Langkahnya tercantum di bawah dengan hasil "–".
...
- **Verifikasi:**
  ...
  - [ ] Login **manager** → buka `/monitoring-kwh` → apakah tombol Tambah/Edit/Hapus tampil? → hasil: – (masih tampil; bagian frontend dikerjakan di Fase 2/3)
  ...
  - [ ] Browser: login operator → ketik `/users` di address bar → halaman 403 → hasil: –
...
- **Verifikasi:**
  ...
  - [ ] Ubah jam sistem ke 01:00 → buka modal tambah kWh Engine → hasil: –
  - [ ] Idem untuk modal kWh Penyulang, Gangguan dan BBM → hasil: –
...
- **Kriteria selesai:** input tidak valid → modal tetap terbuka, isian utuh, pesan error terlihat.
- **Verifikasi:**
  ...
  - [ ] Browser — Users: tambah user dengan email yang sudah ada → modal tetap terbuka dan muncul pesan "email has already been taken" → hasil: –
  - [ ] Browser — Gangguan: kirim data tidak valid → hasil: –
  - [ ] Browser — BBM: kirim nilai negatif / tidak valid → hasil: –
  - [ ] Browser — setelah sukses, modal tertutup dan error hilang → hasil: –
...
- **Verifikasi:**
  ...
  - [x] `npm run build` → hasil: sukses.
  - [ ] Browser: buka edit record BBM → input tanggal disabled (tampak redup) → hasil: –
  - [ ] Browser: simpan edit → jumlah record tidak bertambah → hasil: –
  - [ ] Browser: tambah data untuk tanggal yang sudah ada → modal tetap terbuka dengan pesan error → hasil: –
- **Catatan:** –
...
- **Verifikasi:**
  ...
  - [ ] Browser: badge "Belum Ada Data" tampil → hasil: –
...
- **Verifikasi:**
  - [ ] Ciutkan sidebar → pindah 3 menu → hasil: –
  - [ ] Reload halaman → sidebar tetap tercutkan → hasil: –
  - [ ] Mode gelap → pindah menu / reload → tidak ada kedip terang → hasil: –
  - [ ] Halaman Login & Lupa Password tidak memakai layout → hasil: –
- **Catatan:** –
...
- **Verifikasi:**
  ...
  - [ ] Cek setiap lokasi di tabel BUG-11 dalam **light mode** → semua terbaca → hasil: –
  - [ ] Hover baris tabel di dark mode berfungsi → hasil: –
...
- **File diubah / dibuat:** –
- **Ringkasan perubahan:** –
- **Verifikasi (pada komponen):**
  - [ ] Esc menutup modal → hasil: –
  - [ ] Klik backdrop menutup modal → hasil: –
  - [ ] Fokus awal masuk ke field pertama → hasil: –
  - [ ] Modal panjang bisa di-scroll di layar 375px dengan keyboard terbuka → hasil: –
  - [ ] Klik label memfokuskan input → hasil: –
  - [ ] Tombol loading/disabled terlihat berbeda dan tidak bisa diklik dua kali → hasil: –
- **Catatan:** –
...
- **Ringkasan perubahan:** –
- **Verifikasi:**
  - [ ] Simpan baris paling bawah Monitoring Arus → toast terlihat tanpa scroll → hasil: –
  - [ ] Toast hilang otomatis & bisa ditutup manual → hasil: –
  - [ ] Blok flash lama per halaman sudah dihapus → hasil: –
- **Catatan:** –
...
- **File diubah / dibuat:** –
- **Verifikasi:**
  - [ ] Sidebar memakai `can()` dan hasilnya sama seperti sebelumnya untuk setiap role → hasil: –
...
> Untuk setiap halaman, verifikasi minimal: **light mode, dark mode, lebar 375px (mobile)**, dan role tanpa izin input.
...
- **Ringkasan perubahan:** –
- **Verifikasi:**
  - [ ] BUG-01: isi jam 08.30 & 09.00 → simpan 08.30 → isian 09.00 tetap ada → hasil: –
  - [ ] BUG-01: ada baris belum disimpan → ganti shift → muncul konfirmasi → hasil: –
  - [ ] BUG-01: tombol "Simpan Semua" menyimpan semua baris dirty → hasil: –
  - [ ] BUG-01: error validasi tampil di baris terkait → hasil: –
  - [ ] BUG-14: operator otomatis nama akun bila kosong → hasil: –
  - [ ] BUG-14: tidak ada angka "12"/"16" hardcoded (`grep`) → hasil: –
  - [ ] BUG-14: label sumbu Y dibulatkan → hasil: –
  - [ ] Light / dark / mobile OK → hasil: –
- **Catatan:** –
...
- **Verifikasi:**
  ...
  - [ ] Tombol Edit/Hapus mobile ≥ 40px & berjarak → hasil: –
  - [ ] Role tanpa `monitoring_kwh.input` tidak melihat tombol aksi → hasil: –
  - [ ] Light / dark / mobile OK → hasil: –
- **Catatan:** –
...
- **Verifikasi:**
  ...
  - [ ] Tombol mobile ≥ 40px → hasil: –
  - [ ] Role tanpa `monitoring_engine.input` tidak melihat tombol aksi → hasil: –
  - [ ] Light / dark / mobile OK → hasil: –
- **Catatan:** –
...
- **File diubah:** –
- **Verifikasi:**
  - [ ] Tombol Batal, tombol X, box "Last Modified" terbaca di light mode → hasil: –
  - [ ] Modal bisa di-scroll di mobile → hasil: –
  - [ ] Role tanpa `monitoring_gangguan.manage` tidak melihat tombol aksi → hasil: –
  - [ ] Light / dark / mobile OK → hasil: –
- **Catatan:** –
...
- **Verifikasi:**
  - [ ] Tombol Batal, tombol X, box "Last Modified" terbaca di light mode → hasil: –
  - [ ] Form tidak diawali angka 0 (placeholder) → hasil: –
  - [ ] Role tanpa `monitoring_bbm.input` tidak melihat tombol aksi → hasil: –
  - [ ] Light / dark / mobile OK → hasil: –
- **Catatan:** –
...
- **File diubah:** –
- **Verifikasi:**
  - [ ] Tampilan kartu di 375px, tombol aksi terjangkau → hasil: –
  - [ ] Email & badge role terbaca di light mode → hasil: –
  - [ ] Pencarian ter-debounce (1 request setelah berhenti mengetik) → hasil: –
  - [ ] Toggle status meminta konfirmasi → hasil: –
  - [ ] User baru: password kosong & wajib diisi → hasil: –
- **Catatan:** –
...
- **File diubah:** –
- **Verifikasi:**
  - [ ] Tanggal hari ini tampil → hasil: –
  - [ ] Sub-teks gangguan menampilkan 3 status & jumlahnya cocok dengan total → hasil: –
  - [ ] Label "Update terakhir" (bukan "Shift 09.30") → hasil: –
  - [ ] Titik status feeder abu bila belum ada data → hasil: –
  - [ ] Kartu KPI tidak link ke halaman tanpa izin → hasil: –
  - [ ] Tombol "Lihat Semua Log Gangguan" & deskripsi terbaca di light mode → hasil: –
- **Catatan:** –
...
- **File diubah:** –
- **Verifikasi:**
  - [ ] Operator di 375px: hanya melihat menu yang diizinkan → hasil: –
  - [ ] Admin di 375px: bisa membuka User Management via "Lainnya" → hasil: –
  - [ ] Label terbaca (≥ 10px), maks. 5 slot → hasil: –
- **Catatan:** –
...
- **File diubah:** –
- **Verifikasi:**
  - [ ] Judul tab browser berbeda di setiap halaman → hasil: –
  - [ ] Header menampilkan judul halaman (desktop & mobile) → hasil: –
  - [ ] Timer jam: ditampilkan atau dihapus (tidak ada interval yang tak terpakai) → hasil: –
- **Catatan:** –
...
- [ ] Uji menyeluruh 5 role (admin, manager, tl_operasi, tl_pemeliharaan, operator) di desktop & 375px → hasil: –
- [ ] Tidak ada error di console browser di setiap halaman → hasil: –
